...
G4VPhysicalVolume* WGR16DetectorConstruction::Construct()
{
	...
	// for(G4int i_cu=0; i_cu<nTower_PhiDir; i_cu++)
	{
		...
		Trd_rotM.rotateY(90*deg-dEta);
		Trd_rotM.rotateZ(Trd_phi);
		
		//this codes making overlap... need to be corrected
		G4double move_tri=radius/std::cos(half_dPhi) + 0.5*CuLen_H*std::cos(dEta)+0.5*CuLen_EtaDir*std::sin(dEta);
		G4double tri_z_move=radius*std::tan(dEta)/std::cos(half_dPhi)+0.5*CuLen_H*std::sin(dEta)+0.5*CuLen_EtaDir*std::sin(dEta)*tan(dEta);
